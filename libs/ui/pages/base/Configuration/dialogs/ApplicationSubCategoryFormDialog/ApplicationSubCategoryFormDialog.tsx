...
interface ApplicationSubCategoryFormDialogProps {
  open: boolean;
  editing: IConfigApplicationSubCategory | null;
  /** Other application-sub-category rows for duplicate detection. */
  existingSubCategories?: IConfigApplicationSubCategory[];
  /** Drop-down options for the "Application category" field. Sourced
   * from the existing application-categories list. `value` and `label`
   * are the application category name. */
  applicationCategoryOptions: { value: string; label: string }[];
  onClose: () => void;
  onSave: (data: Partial<IConfigApplicationSubCategory>) => void;
  subtitle?: string;
}
...
const ApplicationSubCategoryFormDialog = ({
  open,
  editing,
  ...
  onSave,
  subtitle,
}: ApplicationSubCategoryFormDialogProps) => {
  const { success } = useNotification();
  const reqError = useFieldError();
  const [form, setForm] = useState<Partial<IConfigApplicationSubCategory>>({});
  ...
  const formRef = useRef<Partial<IConfigApplicationSubCategory>>({});
  ...
  const [touched, setTouched] = useState<{
    applicationCategoryName?: boolean;
  ...
  const updateForm = useCallback(
    (
      patch:
        | Partial<IConfigApplicationSubCategory>
        | ((f: Partial<IConfigApplicationSubCategory>) => Partial<IConfigApplicationSubCategory>),
    ) => {
      formRef.current =
   ...
   *   Application category (applicationCategoryName)        — Required for usability (linked data)
   *   Application sub-category (subCategoryName)           — Yes
   *   Short Description                                    — Yes
   *   Description                                          — No
   *   Internal note                                        — No
   *
   * The "Application sub-category" is the unique key per the spec
   * ("Editable at the time of creation, not allowed to edit the field
   * once it is saved"). We enforce a max length of 30 on it via
   * `inputProps` and the `MAX_NAME` constant. `plainText` enforces
   * alpha-numeric on the underlying stored value.
   */
  const validateRequired = (f: Partial<IConfigApplicationSubCategory>): typeof requiredErrors => {
    const errs: typeof requiredErrors = {};
    if (!String(f.applicationCategoryName ?? '').trim()) errs.applicationCategoryName = 'required';
    if (!plainText(f.subCategoryName ?? '')) errs.subCategoryName = 'required';
   ...
   *
   * Per the spec for the Application Sub-Category section:
   *   - Application category:        Allowed  — skip
   *   - Application sub-category:    Not allowed
   *   - Short Description:           Not allowed (used as tool tip)
   *   - Description:                 Allowed  — skip
   *   - Internal note:               Allowed  — skip
   *
   * NOTE: Application sub-category is the unique key per the spec
   * ("Editable at the time of creation, not allowed to edit the field
   ...
   * row.
   */
  const computeDuplicateMessage = (f: Partial<IConfigApplicationSubCategory>): string | null => {
    const myId = editing?.id;
    const others = existingSubCategories.filter((c) => c.id !== myId);

    const conflicts: string[] = [];

    const nameVal = plainText(f.subCategoryName ?? '');
    if (nameVal && others.some((c) => plainText(c.subCategoryName ?? '') === nameVal)) {
      conflicts.push('Application sub-category');
    }
  ...
  }, [form, open, editing, existingSubCategories]);
  ...
  useEffect(() => {
    ...
    setTouched({});
    setRequiredErrors({});
    const initial: Partial<IConfigApplicationSubCategory> = editing
      ? {
          applicationId: editing.applicationId ?? '',
          applicationName: editing.applicationName ?? '',
          applicationCategoryId: editing.applicationCategoryId ?? '',
          applicationCategoryName: editing.applicationCategoryName ?? '',
  ...
  const handleSubmit = () => {
    ...
    // border / helper text on Application Category, Application Sub-Category,
    // and Short Description when any of them is blank. Mark all required
    ...
    success(
      editing
        ? 'Application sub-category updated successfully'
        : 'Application sub-category added successfully',
    );
  };

  const catError = reqError(
    touched.applicationCategoryName,
    requiredErrors.applicationCategoryName,
  );
  const nameError = reqError(touched.subCategoryName, requiredErrors.subCategoryName);
  const shortError = reqError(touched.shortDescription, requiredErrors.shortDescription);

  // ── Application Category — search-style autocomplete ─────────────────────
  // Mirrors the Business Category pattern used by the Service Line dialog:
  //   - TextField with a search/clear end adornment
  //   - Floating Paper + List of matching application-category options
  //   - Stored value is the application category name
  const [catInput, setCatInput] = useState<string>('');
  const [catOptionsOpen, setCatOptionsOpen] = useState(false);
  const [catLoading, setCatLoading] = useState(false);
  const catDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const [catFiltered, setCatFiltered] = useState<{ value: string; label: string }[]>([]);

  useEffect(() => {
    const stored = String(form.applicationCategoryName ?? '');
    if (stored !== catInput) {
      setCatInput(stored);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [form.applicationCategoryName]);

  const catSearch = useCallback(
    (query: string) => {
      const q = query.trim().toLowerCase();
      if (!q) return applicationCategoryOptions;
      return applicationCategoryOptions.filter((o) =>
        String(o.label ?? '')
          .toLowerCase()
          .includes(q),
      );
  ...
  const handleCatInputChange = useCallback(
    (value: string) => {
      setCatInput(value);
      if (catDebounceRef.current) clearTimeout(catDebounceRef.current);
      catDebounceRef.current = setTimeout(() => {
        setCatLoading(true);
        const next = catSearch(value);
        setCatFiltered(next);
        setCatOptionsOpen(next.length > 0);
        setCatLoading(false);
      }, 200);
    },
    [catSearch],
  ...
  const handleCatSelect = useCallback(
    (opt: { value: string; label: string }) => {
      setCatInput(opt.label);
      setCatOptionsOpen(false);
      setCatFiltered([]);
      setTouched((t) => ({ ...t, applicationCategoryName: true }));
      updateForm((f) => ({
        ...f,
        applicationCategoryId: opt.value,
        applicationCategoryName: opt.value,
      }));
    },
    [updateForm],
  );
  ...
  const handleCatClear = useCallback(() => {
    ...
    setCatFiltered([]);
    setCatOptionsOpen(false);
    updateForm((f) => ({ ...f, applicationCategoryId: '', applicationCategoryName: '' }));
  }, [updateForm]);
  ...
  return (
    <ConfigFormDialog
      ...
      isEdit={!!editing}
      icon={<SubdirectoryArrowRightIcon sx={{ color: '#fff', fontSize: '1.1rem' }} />}
      accent={APC_ACCENT}
    ...
    >
      {/* Duplicate Alert — single dialog-level message. Per spec, only
          Application sub-category and Short Description must be unique. */}
      {duplicateAlert && (
      ...
      {/* Application Category — required search field. Sourced from
          Categorization > Application category (tab) > Application
          category name. */}
      <Box>
        <Box sx={{ position: 'relative' }}>
          <TextField
            ...
            onFocus={() => {
              const next = catSearch(catInput);
              setCatFiltered(next);
              if (next.length > 0) setCatOptionsOpen(true);
            }}
            onBlur={() => {
              setTouched((t) => ({ ...t, applicationCategoryName: true }));
              setTimeout(() => setCatOptionsOpen(false), 200);
            }}
            fullWidth
            size='small'
            required
          ...
          />
          {catOptionsOpen && catFiltered.length > 0 && (
            <Paper
              elevation={4}
              sx={{
                position: 'absolute',
        ...
        <Typography
          variant='caption'
          sx={{
            color: catError ? '#d32f2f' : 'transparent',
            fontSize: '0.75rem',
            mt: 0.5,
            ml: 1.75,
            display: 'block',
      ...
      {/* Application sub-category — required, alpha-numeric (max 30),
          locked after save. The spec says "Editable at the time of
          creation, not allowed to edit the field once it is saved" —
          so we disable the input when editing. */}
      <Box>
        <TextField
          label='Application sub-category'
          size='small'
          value={form.subCategoryName ?? ''}
          onChange={(e) => updateForm((f) => ({ ...f, subCategoryName: e.target.value }))}
          onBlur={() => setTouched((t) => ({ ...t, subCategoryName: true }))}
          placeholder='e.g. UI Bug, Backend Issue'
          inputProps={{
...
export default ApplicationSubCategoryFormDialog;
