...
import { Apps as AppsIcon, Clear as ClearIcon, Search as SearchIcon } from '@mui/icons-material';
...
interface ApplicationsFormDialogProps {
  open: boolean;
  editing: IConfigApplication | null;
  /** Other application rows for duplicate detection. */
  existingApplications?: IConfigApplication[];
   ...
   * Manager Level 1, Manager Level 2). Sourced from
  ...
  onClose: () => void;
  onSave: (data: Partial<IConfigApplication>) => void;
  subtitle?: string;
}
...
const ApplicationsFormDialog = ({
  ...
  const [form, setForm] = useState<Partial<IConfigApplication>>({});
  ...
  const formRef = useRef<Partial<IConfigApplication>>({});
  ...
  const updateForm = useCallback(
    (
      patch:
        | Partial<IConfigApplication>
        | ((f: Partial<IConfigApplication>) => Partial<IConfigApplication>),
    ) => {
      formRef.current =
   ...
   *   Service line (serviceLineName)   — Required for usability (linked data)
   *   Application name (name)          — Yes
   ...
   *   Internal note                    — No
   *
   * The "Application name" is the unique key per the spec ("Editable at
   * the time of creation, not allowed to edit the field once it is
   ...
   */
  const validateRequired = (f: Partial<IConfigApplication>): typeof requiredErrors => {
   ...
   *
   * Per the spec for the Application section:
   *   - Service line:        Allowed  — skip
   *   - Application name:    Not allowed
   *   - Short Description:   Not allowed (used as tool tip)
   *   - Description:         Allowed  — skip
   *   - Application lead:    Allowed  — skip
   *   - Manager Level 1:    Allowed  — skip
   *   - Manager Level 2:    Allowed  — skip
   *   - Internal note:       Allowed  — skip
   *
   * NOTE: Application Name is the unique key per the spec ("Editable at
   * the time of creation, not allowed to edit the field once it is
   * saved"). Even though the Name field is disabled when editing, we
   * still include the name in the duplicate check so the user gets
   * feedback if the (locked) value collides with another row.
   */
  const computeDuplicateMessage = (f: Partial<IConfigApplication>): string | null => {
    const myId = editing?.id;
    const others = existingApplications.filter((a) => a.id !== myId);

    const conflicts: string[] = [];

    const nameVal = plainText(f.name ?? '');
    if (nameVal && others.some((a) => plainText(a.name ?? '') === nameVal)) {
      conflicts.push('Application Name');
    }

    const shortVal = plainText(f.shortDescription ?? '');
    if (shortVal && others.some((a) => plainText(a.shortDescription ?? '') === shortVal)) {
  ...
  }, [form, open, editing, existingApplications]);
  ...
  useEffect(() => {
    ...
    setTouched({});
    setRequiredErrors({});
    const initial: Partial<IConfigApplication> = editing
      ? {
          ...
          applicationLead: editing.applicationLead ?? '',
  ...
  const handleSubmit = () => {
    ...
    // border / helper text on Service line, Application Name, and Short
    ...
    success(editing ? 'Application updated successfully' : 'Application added successfully');
  };
  ...
  //   - Floating Paper + List of matching service-line options
  //   - Stored value is the service line name
  ...
  const handleSlSelect = useCallback(
    (opt: { value: string; label: string }) => {
      ...
      updateForm((f) => ({ ...f, serviceLineId: opt.value, serviceLineName: opt.value }));
    },
    [updateForm],
  );
  ...
  const handleSlClear = useCallback(() => {
    ...
    setSlFiltered([]);
    setSlOptionsOpen(false);
    updateForm((f) => ({ ...f, serviceLineId: '', serviceLineName: '' }));
  }, [updateForm]);

  // ── Application lead — search-style autocomplete ─────────────────────────
  const [appLeadInput, setAppLeadInput] = useState<string>('');
  const [appLeadOptionsOpen, setAppLeadOptionsOpen] = useState(false);
  const [appLeadLoading, setAppLeadLoading] = useState(false);
  const appLeadDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const [appLeadFiltered, setAppLeadFiltered] = useState<
    { value: string; label: string; subtitle?: string }[]
  >([]);

  useEffect(() => {
    const stored = String(form.applicationLead ?? '');
    if (stored !== appLeadInput) {
      setAppLeadInput(stored);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [form.applicationLead]);

  const appLeadSearch = useCallback(
    (query: string) => {
      const q = query.trim().toLowerCase();
  ...
  const handleAppLeadInputChange = useCallback(
    (value: string) => {
      setAppLeadInput(value);
      if (appLeadDebounceRef.current) clearTimeout(appLeadDebounceRef.current);
      appLeadDebounceRef.current = setTimeout(() => {
        setAppLeadLoading(true);
        const next = appLeadSearch(value);
        setAppLeadFiltered(next);
        setAppLeadOptionsOpen(next.length > 0);
        setAppLeadLoading(false);
      }, 200);
    },
    [appLeadSearch],
  );

  const handleAppLeadSelect = useCallback(
    (opt: { value: string; label: string }) => {
      setAppLeadInput(opt.label);
      setAppLeadOptionsOpen(false);
      setAppLeadFiltered([]);
      updateForm((f) => ({ ...f, applicationLead: opt.value }));
    },
    [updateForm],
  );

  const handleAppLeadClear = useCallback(() => {
    setAppLeadInput('');
    setAppLeadFiltered([]);
    setAppLeadOptionsOpen(false);
    updateForm((f) => ({ ...f, applicationLead: '' }));
  }, [updateForm]);
                ...
                <ListItem key={opt.value} disablePadding>
                  <ListItemButton
                    ...
                    sx={{
                      py: 1,
                      px: 1.5,
                      '&:hover': { bgcolor: alpha(APP_ACCENT, 0.08) },
                    }}
                  >
                    <ListItemText
                      primary={opt.label}
  ...
  return (
    ...
    >
      {/* Duplicate Alert — single dialog-level message. Per spec, only
          Application Name and Short Description must be unique. The Alert
      ...
      {/* Service line — required search field. Sourced from
          Categorization > Service line (tab) > Service line name. */}
      {renderSearchField({
        label: 'Service line',
        placeholder: 'Search service lines...',
        input: slInput,
        optionsOpen: slOptionsOpen,
        loading: slLoading,
        filtered: slFiltered as { value: string; label: string; subtitle?: string }[],
        onChange: handleSlInputChange,
        onSelect: handleSlSelect,
        onClear: handleSlClear,
        onFocus: () => {
          const next = slSearch(slInput);
          setSlFiltered(next);
          if (next.length > 0) setSlOptionsOpen(true);
        },
        onClosePopover: () => setSlOptionsOpen(false),
        required: true,
        error: slError,
      })}

      {/* Application Name — required, alpha-numeric (max 30), locked
      ...
      <Box>
        <TextField
          ...
          placeholder='e.g. ServiceOps Portal'
          inputProps={{
      ...
      {/* Application lead — optional. Sourced from User Management > All
          Users. Stored value is the user's full name. */}
      {renderSearchField({
        label: 'Application lead',
        placeholder: 'Search users...',
        input: appLeadInput,
        optionsOpen: appLeadOptionsOpen,
        loading: appLeadLoading,
        filtered: appLeadFiltered,
        onChange: handleAppLeadInputChange,
        onSelect: handleAppLeadSelect,
        onClear: handleAppLeadClear,
        onFocus: () => {
          const next = appLeadSearch(appLeadInput);
          setAppLeadFiltered(next);
          if (next.length > 0) setAppLeadOptionsOpen(true);
        },
        onClosePopover: () => setAppLeadOptionsOpen(false),
      })}
