...
const BusinessCategories = () => {
  ...
  useEffect(() => {
    if (apiCat?.businessCategories) setRows(apiCat.businessCategories);
  }, [apiCat]);
...
const ServiceLines = () => {
  ...
  useEffect(() => {
    if (apiCat?.serviceLines) setRows(apiCat.serviceLines);
  }, [apiCat]);
  ...
  return (
    <Box sx={{ mt: 2 }}>
      ...
      >
        <Box className={classes.toolbarButtons}>
          {!selectedRow ? (
            <Tooltip title='Add support line'>
              <Button
                size='small'
                variant='contained'
                startIcon={<AddIcon />}
  ...
  return (
    <Box sx={{ mt: 2 }}>
      ...
      >
        <Box className={classes.toolbarButtons}>
          {!selectedRow ? (
            <Tooltip title='Add billing code'>
              <Button
                size='small'
                variant='contained'
                startIcon={<AddIcon />}
...
const Applications = () => {
  ...
  useEffect(() => {
    if (apiCat?.applications) setRows(apiCat.applications);
  }, [apiCat]);
      ...
      format: (v): React.ReactNode => (
        <Chip
          ...
          sx={{
            ...
            fontSize: '0.72rem',
            height: 20,
            bgcolor: v ? alpha('#059669', 0.1) : alpha('#64748b', 0.08),
            color: v ? '#059669' : 'text.secondary',
          }}
        />
      ),
    },
...
const EMPTY_QAP: Omit<IConfigApproval, 'id'> = {
  approverName: '',
  approverRole: '',
  approvalOrder: 1,
  isRequired: true,
...
const ApplicationQueues = () => {
  ...
  useEffect(() => {
    if (apiCat?.queues) setRows(apiCat.queues);
  }, [apiCat]);
...
const ApplicationCategories = () => {
  ...
  useEffect(() => {
    if (apiCat?.applicationCategories) setRows(apiCat.applicationCategories);
  }, [apiCat]);
...
const ApplicationSubCategories = () => {
  ...
  useEffect(() => {
    if (apiCat?.applicationSubCategories) setRows(apiCat.applicationSubCategories);
  }, [apiCat]);
...
const ApplicationNumberSequences = () => {
  ...
  useEffect(() => {
    if (apiCat?.applicationNumberSequences) setRows(apiCat.applicationNumberSequences);
  }, [apiCat]);
