import type { IConfigConversionReasonCode } from '@serviceops/interfaces';
...
/**
 * Returns field-level errors when the in-progress Conversion Reason Code row
 ...
 */
export const validateConversionDuplicate = (
  ...
  const rows = data as IConfigConversionReasonCode[];
  const editing = editingRow as IConfigConversionReasonCode | null;
  ...
  return {
    _form: `Conversion Reason Code "${form.name}" already exists. Please use a different name.`,
  };
};
