import type { IConfigResolutionCode } from '@serviceops/interfaces';
...
/**
 * Returns field-level errors when the in-progress Resolution Code row
 ...
 */
export const validateResolutionDuplicate = (
  ...
  const rows = data as IConfigResolutionCode[];
  const editing = editingRow as IConfigResolutionCode | null;
  ...
  return {
    _form: `Resolution Code "${form.name}" already exists. Please use a different name.`,
  };
};
