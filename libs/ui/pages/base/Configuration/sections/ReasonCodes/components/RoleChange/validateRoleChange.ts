import type { IConfigRoleChangeReasonCode } from '@serviceops/interfaces';
...
/**
 * Returns field-level errors when the in-progress Role Change Reason Code row
 ...
 */
export const validateRoleChangeDuplicate = (
  ...
  const rows = data as IConfigRoleChangeReasonCode[];
  const editing = editingRow as IConfigRoleChangeReasonCode | null;
  ...
  return {
    _form: `Role Change Reason Code "${form.name}" already exists. Please use a different name.`,
  };
};
