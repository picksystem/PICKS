...
import { IConfigProject } from '@serviceops/interfaces';
...
const ProjectSearchField = ({
  ...
  const { clientsAndProjects } = useConfiguration();
  ...
  const projects: IConfigProject[] = clientsAndProjects?.projects ?? [];
    ...
    (query: string) => {
      ...
      const options = q
        ? projects
            ...
            .map((p) => ({ id: p.projectId, label: p.projectName }))
        : projects.map((p) => ({ id: p.projectId, label: p.projectName }));
  ...
  return (
    <Box ref={containerRef} sx={{ position: 'relative', ...sx }}>
      <TextField
        ...
        required={required}
        disabled={disabled}
        placeholder={loading ? 'Loading...' : 'Search projects...'}
        value={inputValue}
