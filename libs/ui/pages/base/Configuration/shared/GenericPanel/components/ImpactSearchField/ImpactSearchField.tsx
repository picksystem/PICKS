...
import { IConfigImpactLevel } from '@serviceops/interfaces';
import { useConfiguration } from '@serviceops/pages/base/Configuration/hooks/useConfiguration';

interface ImpactSearchFieldProps {
  label?: string;
  value: string;
...
const ImpactSearchField = ({
  ...
  const impacts: IConfigImpactLevel[] = priorities?.impactLevels?.filter((i) => i.isActive) ?? [];
  ...
  const buildOptions = useCallback(
    (query: string) => {
      ...
      const options = q
        ? impacts
            .filter(
              (i) => i.displayName.toLowerCase().includes(q) || i.name.toLowerCase().includes(q),
            )
            .map((i) => ({ id: i.name, label: i.displayName }))
        : impacts.map((i) => ({ id: i.name, label: i.displayName }));
  ...
  return (
    <Box ref={containerRef} sx={{ position: 'relative', ...sx }}>
      <TextField
        ...
        required={required}
        disabled={disabled}
        placeholder={loading ? 'Loading...' : 'Search impact levels...'}
        value={inputValue}
