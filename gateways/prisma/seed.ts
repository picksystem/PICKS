...
async function clearAndSeed() {
  ...
  await pool.query(`
    INSERT INTO "User" ("firstName", "lastName", "email", "password", "phone", "workLocation", "department", "businessUnit", "employeeId", "managerName", "name", "role", "status", "source", "timezone", "dateFormat", "timeFormat", "language", "slaWorkingCalendar", "slaExceptionGroup", "lastActivityAt", "failedLoginAttempts", "isActive", "mustResetPassword", "firstActivationDate", "createdAt", "updatedAt")
      VALUES ('Admin', 'User', 'admin@picks.com', '${adminPassword}', '+1-555-0001', 'New York - HQ', 'IT Administration', 'Technology', 'EMP001', 'CTO Office', 'Admin User', 'admin', 'active', 'admin', 'America/New_York', 'MM/DD/YYYY', '12h', 'en', 'Standard 8x5', 'US Federal Holidays', NOW() - INTERVAL '15 minutes', 0, true, false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
    INSERT INTO "User" ("firstName", "lastName", "email", "password", "phone", "workLocation", "department", "businessUnit", "employeeId", "managerName", "name", "role", "status", "source", "timezone", "dateFormat", "timeFormat", "language", "slaWorkingCalendar", "slaExceptionGroup", "lastActivityAt", "failedLoginAttempts", "isActive", "mustResetPassword", "firstActivationDate", "createdAt", "updatedAt")
      VALUES ('Regular', 'User', 'user@picks.com', '${userPassword}', '+1-555-0002', 'Chicago - Branch', 'Finance', 'Corporate Services', 'EMP002', 'Jane Manager', 'Regular User', 'user', 'active', 'admin', 'America/Chicago', 'MM/DD/YYYY', '12h', 'en', 'Standard 8x5', 'US Federal Holidays', NOW() - INTERVAL '3 hours', 0, true, false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
    INSERT INTO "User" ("firstName", "lastName", "email", "password", "phone", "workLocation", "department", "businessUnit", "employeeId", "managerName", "reasonForAccess", "name", "role", "status", "source", "timezone", "dateFormat", "timeFormat", "language", "slaWorkingCalendar", "slaExceptionGroup", "lastActivityAt", "failedLoginAttempts", "isActive", "mustResetPassword", "firstActivationDate", "createdAt", "updatedAt")
      VALUES ('Consultant', 'User', 'consultant@picks.com', '${consultantPassword}', '+1-555-0003', 'Remote', 'External Consulting', 'Professional Services', 'CON001', 'Project Lead', 'Assigned to PICKS implementation project', 'Consultant User', 'consultant', 'active', 'admin', 'America/Los_Angeles', 'MM/DD/YYYY', '24h', 'en', 'Extended 10x5', 'US Federal Holidays', NOW() - INTERVAL '1 day', 0, true, false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
  `);
  ...
  await pool.query(`
    INSERT INTO "AdminIncident" ("number", "caller", "callerPhone", "callerEmail", "callerLocation", "callerDepartment", "businessCategory", "serviceLine", "application", "shortDescription", "description", "impact", "urgency", "priority", "channel", "status", "assignmentGroup", "primaryResource", "createdBy", "isRecurring", "notes", "createdAt", "updatedAt")
      VALUES ('INC0001001', 'John Doe', '+1-555-0101', 'john.doe@company.com', 'New York - HQ', 'Finance', 'Financial Services', 'Core Banking', 'Payment Gateway', 'Payment gateway returning timeout errors', 'Multiple users experiencing timeout errors when processing payments through the gateway.', 'high', 'high', '1-Critical', 'phone', 'new', 'Payment Support Team', 'Alice Johnson', 'admin@picks.com', false, 'Escalated to P1 due to business impact.', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
    ...
    INSERT INTO "AdminIncident" ("number", "caller", "callerEmail", "callerLocation", "callerDepartment", "businessCategory", "serviceLine", "application", "shortDescription", "description", "impact", "urgency", "priority", "channel", "status", "assignmentGroup", "createdBy", "isRecurring", "notes", "createdAt", "updatedAt")
      VALUES ('INC0001006', 'Lisa Chen', 'lisa.chen@company.com', 'Seattle - Remote', 'Engineering', 'Technology', 'Software Development', 'Internal Wiki', 'Wiki page not rendering correctly', 'Reported wiki page rendering issue. Turned out to be a browser cache problem.', 'low', 'low', '5-Planning', 'portal', 'cancelled', 'Service Desk', 'user@picks.com', false, 'Cancelled - user resolved by clearing browser cache.', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
    ...
    INSERT INTO "AdminIncident" ("number", "caller", "callerPhone", "callerEmail", "callerLocation", "callerDepartment", "businessCategory", "serviceLine", "application", "shortDescription", "description", "impact", "urgency", "priority", "channel", "status", "assignmentGroup", "createdBy", "isRecurring", "isMajor", "notes", "draftExpiresAt", "createdAt", "updatedAt")
      VALUES ('INC0001010', 'Ryan Martinez', '+1-555-0110', 'ryan.martinez@company.com', 'Miami - Office', 'Sales', 'Sales Operations', 'CRM', 'Salesforce', 'Draft - CRM field customization request', 'Request to add custom fields for new product line tracking in Salesforce. Awaiting manager approval.', 'low', 'low', '5-Planning', 'email', 'draft', 'CRM Admin Team', 'user@picks.com', false, false, 'Draft - pending department head sign-off.', CURRENT_TIMESTAMP + INTERVAL '7 days', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
  `);
